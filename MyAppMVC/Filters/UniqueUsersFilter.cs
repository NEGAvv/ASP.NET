...

{
    public class UniqueUsersFilter : Attribute, IActionFilter
    {
        ...
        private static string logFilePath = Path.Combine(Directory.GetCurrentDirectory(), "Logs", "unique_users_amount_log.txt");
        ...
        private string GetUserIdFromContext(ActionExecutedContext context)
        {
            var userId = context.HttpContext.Request.Cookies["userId"];
            Console.WriteLine($"User cookie: {userId}");
            return userId;
        }
